...
#ifndef STEPPINGLOOPSPHERE_HH
#define SteppingLoopSphere_HH

/**
 * @file    SteppingLoopSphere.hh
 * @class   SteppingLoopSphere
 * @author  M. Novak
 * @date    July 2023
 *
 ...
 * (by the `SteppingLoopSphere::GammaStepper(G4HepEmTLData&, G4HepEmState&, TrackStack&,
 * GeometrySphere&, Results&, int)` and `SteppingLoopSphere::ElectronStepper(G4HepEmTLData&,
 * G4HepEmState&, TrackStack&, GeometrySphere&, Results&, int)` respectively). At each
 * step:
 ...
 *   `SteppingLoopSphere::StackSecondaries(G4HepEmTLData&, TrackStack&, G4HepEmTrack&)`
 *   method)
 * - information (e.g. energy deposit) might be collected at the end of each
 *   simulation step (by calling the `SteppingLoopSphere::SteppingAction(Results&,
 *   const G4HepEmTrack&, const Sphere*, G4double, int, int, int, int)` method )
 *
 * **A bit more details**:
 *
 ...
 * the `SteppingLoopSphere::GammaStepper()` and `SteppingLoopSphere::ElectronStepper()`.
...
public:
   ...
   * @param theGeometrySphere the GeometrySphere of the application in which the input track history is simulated
   ...
   */
  static void GammaStepper(G4HepEmTLData& theTLData, G4HepEmState& theState, TrackStack& theTrackStack, GeometrySphere& theGeometrySphere, Results& theResult, int eventID);
   ...
   */
  static void ElectronStepper(G4HepEmTLData& theTLData, G4HepEmState& theState, TrackStack& theTrackStack, GeometrySphere& theGeometrySphere, Results& theResult, int eventID);


private:
  SteppingLoopSphere() = delete;
   ...
   */
  static void SteppingAction(Results& theResult, const G4HepEmTrack& theTrack, const Sphere* currentVolume, G4double currentPhysStepLength, int indxLayer, int indxAbsorber, int eventID, int stepID);
