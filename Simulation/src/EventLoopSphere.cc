...
void EventLoopSphere::ProcessEvents(G4HepEmTLData& theTLData, G4HepEmState& theState, PrimaryGenerator& thePrimaryGenerator, GeometrySphere& theGeometrySphere, Results& theResult, int numEventToSimulate, int verbosity) {
  //
  ...
  // report progress
  if (verbosity > 0) {
    std::cout << " --- EventLoopSphere::ProcessEvents: starts simulation of N = " << numEventToSimulate << " events..." << std::endl;
  }
  //
  ...
  while (eventID < numEventToSimulate) {
    ...
    BeginOfEventAction(theResult, eventID, primaryTrack, theGeometrySphere, thePrimaryGenerator);
    ...
    while ( (trackType = theTrackStack.GetTypeOfNextTrack()) > -2 ) {
      ...
      if (trackType == 0) { // the next track is a gamma
        SteppingLoopSphere::GammaStepper(theTLData, theState, theTrackStack, theGeometrySphere, theResult, eventID);
      } else {              // the next track is an e- or e+
        SteppingLoopSphere::ElectronStepper(theTLData, theState, theTrackStack, theGeometrySphere, theResult, eventID);
      }
  ...
  if (verbosity > 0) {
    std::cout << " --- EventLoopSphere::ProcessEvents: completed simulation within t = " << theTime << " [s]" << std::endl;
  }
}


void EventLoopSphere::BeginOfEventAction(Results& theResult, int eventID, const G4HepEmTrack& thePrimaryTrack, GeometrySphere& theGeometrySphere, PrimaryGenerator& thePrimaryGenerator) {
  // reset all per-event accumulators in results, i.e. that are used to accumulate data during one event
    ...
    theResult.pThicknessAbsorber = theGeometrySphere.GetAbsThick();
    G4double::getTape().registerInput(theResult.pThicknessAbsorber);
    theGeometrySphere.SetAbsThick(theResult.pThicknessAbsorber);
    theResult.pThicknessGap = theGeometrySphere.GetGapThick();
    G4double::getTape().registerInput(theResult.pThicknessGap);
    theGeometrySphere.SetGapThick(theResult.pThicknessGap);
...
void EventLoopSphere::EndOfEventAction(Results& theResult, int eventID) {
...
void EventLoopSphere::BeginOfTrackingAction(Results& theResult, G4HepEmTrack& theTrack) {
...
void EventLoopSphere::EndOfTrackingAction(Results& /*theResult*/, G4HepEmTrack& /*theTrack*/) {
